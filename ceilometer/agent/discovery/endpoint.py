...
class EndpointDiscovery(plugin.DiscoveryBase):
    ...
    def discover(manager, param=None):
        ...
        if not endpoints:
            ...
            return []
        LOG.debug(_LW('Tony : service_type=%s'), param)
        LOG.debug(_LW('Tony : interface=%s'), cfg.CONF.service_credentials.interface)
        LOG.debug(_LW('Tony : region_name=%s'), cfg.CONF.service_credentials.region_name)
