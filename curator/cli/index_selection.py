...
def indices(ctx, newer_than, older_than, prefix, suffix, time_unit,
    ...
    # Set main_timeout to match 'timeout' if less than or equal to 300 seconds,
    # otherwise set to 300.  Also, set this before overriding the timeout.
    main_timeout = ctx.parent.parent.params['timeout'] if ctx.parent.parent.params['timeout'] <= 300 else 300
    main_timeout = main_timeout * 1000 # This value is in milliseconds, at least in Python.
            ...
            if ctx.parent.parent.params['dry_run']:
                ...
                if len(to_csv(working_list)) > 3072 and not ctx.parent.info_name == 'snapshot':
                    ...
                    for l in index_lists:
                        retval = do_command(client, ctx.parent.info_name, l, ctx.parent.params, main_timeout)
                        if not retval:
                            success = False
                        time.sleep(2) # Pause in between iterations
                    exit_msg(success)
                else:
                    retval = do_command(client, ctx.parent.info_name, working_list, ctx.parent.params, main_timeout)
                    exit_msg(retval)
